...
router.post('/', async (req, res) => {
  const { indicador_id, periodo_inicio, periodo_fim, meta, meta_atingida, observacao } = req.body;
  ...
  try {
    const [r] = await db.query(
      `INSERT INTO resultados (empresa_id, indicador_id, periodo_inicio, periodo_fim, meta, meta_atingida, percentual_atingido, observacao)
       VALUES (?,?,?,?,?,?,?,?)`,
      [req.usuario.empresa_id, indicador_id, periodo_inicio, periodo_fim, meta, meta_atingida, percentual_atingido, observacao || null]
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const [r] = await db.query(
      `UPDATE resultados SET indicador_id=?, periodo_inicio=?, periodo_fim=?,
        meta=?, meta_atingida=?, percentual_atingido=?, observacao=?
       WHERE id=? AND empresa_id=?`,
      [indicador_id, periodo_inicio, periodo_fim, meta, meta_atingida, percentual_atingido, observacao || null,
